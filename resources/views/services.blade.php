            ...
            @if(isset($services) && count($services) > 0)
                @foreach($services as $serv)
                    <div class="bg-white border border-[#EEF7F4] rounded-[24px] overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 flex flex-col justify-between p-8">
                        ...
                        <div>
                            <a href="{{ filled($serv->slug) ? route('public.services.show', ['slug' => $serv->slug]) : '#services' }}" class="inline-flex items-center text-sm font-semibold text-[#006F5C] hover:text-[#CC205C] transition-colors">
